/**
 * Daily Challenge System for Hopper
 *
 * Provides date-based seeded gameplay so everyone plays the same
 * traffic patterns each day.
 */
...
export function seedForDate(dateStr: string): number {
  ...
  return Math.abs(
    (year * 31337 + month * 1337 + day * 37) | 0
  );
}
...
const DAILY_STORAGE_KEY = 'hopper_daily_leaderboard';
...
export class DailyLeaderboard {
  ...
  static recordScore(
    ...
    entries.push({ name, score, level, frogsHome, completedAt: now });
